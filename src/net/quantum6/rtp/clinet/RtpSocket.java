...
public class RtpSocket implements Runnable {
    private final static String TAG = RtpSocket.class.getCanonicalName();
    ...
    private LinkedBlockingDeque<byte[]> tcpBuffer = new LinkedBlockingDeque<byte[]>();
            ...
            public void run() {
                ...
                while (!Thread.interrupted() && !isStoped) {
                    try {
                        ...
                        offset = 0;
//                        Log.d("myapp","rtspBuffer.inNextPacket="+rtspBuffer.inNextPacket);
    ...
    private void analysisOnePacket(byte[] data, int offset){
        ...
        byte[] buffer;
        datalen = data.length-tmp[0];//offset
//        Log.d("myapp","tmp1="+tmp[1]);
//        Log.d("myapp","datalen="+data.length);

        while(tmp[1] != -1) {//lenght
            if(tmp[1] == -2) {
                rtspBuffer.inNextPacket = true;
                packetFlag = true;
                break;
    ...
    private int[] getRtspFrameInfo(byte[] data,int offset){
        ...
        for(mOffset = offset; mOffset< data.length-1; ++mOffset){
            ...
            if(data[mOffset] == 0x24 && (int)data[mOffset+1] == -1) {
                haveRtspFrame = true;
                break;
            }
            haveRtspFrame = false;
    ...
    private void startUdpReading() throws IOException {
        ...
        RtpStream.receiveData(buffer, mUdpPackets.getLength());
    ...
    @Override
    public void run() {
        Log.d(TAG, "start to get rtp data via socket...="+isTcptranslate);
        try {
            if(isTcptranslate) {
                tcpRecombineThread();
                if(useRtspTcpSocket) {

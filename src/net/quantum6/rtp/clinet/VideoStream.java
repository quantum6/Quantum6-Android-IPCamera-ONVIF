...
public abstract class VideoStream extends RtpStream {
	//private final static String TAG = VideoStream.class.getCanonicalName();
    ...
    protected RtspClientOnvif.SDPInfo mSDPinfo;
    ...
    protected void recombinePacket(RtpStream.StreamPacks streamPacks) {
        ...
        else if (streamPacks.pt == 33)
        {
        	TupRecombine(streamPacks);
        }
    }

    //从参考代码来看，是每188个字节一组。    
    private void TupRecombine(StreamPacks streamPacks)
    {
    	//byte[] data = streamPacks.data;
    	//8bit，0x47
    	//1bit, 0
    ...
    private void H264PacketRecombine(StreamPacks streamPacks) {
        ...
        //Log.d(TAG, "nalType=" + nalType+", "+packFlag+", "+streamPacks.data.length);
        switch (nalType) {

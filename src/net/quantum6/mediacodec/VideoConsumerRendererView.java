 ...
 * OpenGL Surface view
 */
public final class VideoConsumerRendererView extends GLSurfaceView implements GLSurfaceView.Renderer, VideoConsumerable {
	
	private final static String TAG = VideoConsumerRendererView.class.getCanonicalName();
    ...
    public VideoConsumerRendererView(Context context, AttributeSet attrs)
    {
        super(context, attrs);
    }
    
    public VideoConsumerRendererView(Context context)
    {
    	super(context);
    }
    
	public void init(Context context, boolean fullScreenRequired, ByteBuffer buffer, int bufferWidth, int bufferHeight) {
...
//    		mViewX = mViewY = 0;
            ...
            final float bufferRatio = 1f * mBufferWidthY / mBufferHeightY;
            final float viewRatio   = 1f * width / height;
            if (viewRatio > bufferRatio) {
                // 宽度充满，裁剪高度
                mViewWidth = width;
                mViewHeight = (int) (width / bufferRatio);
                mViewX = 0;
                mViewY = ((height - mViewHeight) >> 1);
            } else {
                // 高度充满，裁剪宽度
                mViewWidth = (int) (height * bufferRatio);
                mViewHeight = height;
